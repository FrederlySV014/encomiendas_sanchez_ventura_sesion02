...
STATICFILES_DIRS = [
    BASE_DIR / 'static',   # <- aquí pones tus CSS, JS e imágenes
]

# Carpeta donde se juntan todos al hacer collectstatic (PRODUCCIÓN)
STATIC_ROOT = BASE_DIR / 'staticfiles'

# Archivos subidos por el usuario
# (formularios con FileField/ImageField)
MEDIA_URL = '/media/'
...
REST_FRAMEWORK = {
    ...
    # ── Control de tasa de peticiones (Throttling) ────────────────────
...
SIMPLE_JWT = {
    'ACCESS_TOKEN_LIFETIME':
        timedelta(minutes=60),  # token expira en 1 hora

    'REFRESH_TOKEN_LIFETIME':
        timedelta(days=7),      # refresh expira en 7 días

    'ROTATE_REFRESH_TOKENS':
        True,                   # rotar el refresh en cada uso

    'BLACKLIST_AFTER_ROTATION':
        True,                   # agregar token a blacklist al rotar

    'AUTH_HEADER_TYPES':
        ('Bearer',),            # Authorization: Bearer <token>

    'USER_ID_FIELD':
        'id',                  # campo que usa para identificar usuario

    'USER_ID_CLAIM':
        'user_id',              # claim en el token
}

# ── Configuración de desarrollo ─────────────────────────────────
# Permitir cualquier origen (SOLO EN DESARROLLO)
...
# Permitir envío de cookies (necesario para HttpOnly JWT)
...
# ── Configuración de Silk (Profiling de DB) ──────────────────────
if DEBUG:
    INSTALLED_APPS += ['silk']
